    ...
    for key in petlabel_dic:
       ...
       if found >= 0:
           ...
           # found within a word/term not a label existing on its own 
               ...
               print("\n** Check: found=",found,"\ntruth=",truth, " ",len(truth),
                     "\nmodel_label=",model_label, " ",len(model_label),
                     "\nstart-char=",model_label[found - 1], " ", found-1,
                     "\nend-char=",model_label[found + len(truth): found+len(truth)+1], " ", found+len(truth)+1,
                     "\n")
               if key not in results_dic:
